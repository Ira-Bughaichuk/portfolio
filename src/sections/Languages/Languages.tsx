        ...
        <ul className="flex gap-6 justify-center">
            {languagesList && languagesList.map(({languages, level, path}, index) => (
              <li key={index} className="flex flex-col gap-[10px] ">
                ...
                {path !== '' ? (<Link href={path} className="font-medium leading-[18px] md:leading-[21.6px] xl:leading-6 text-color-accent-primary underline inline-flex justify-end">See more</Link>) : null}
              </li>
            ))}
        </ul>
      </div>
