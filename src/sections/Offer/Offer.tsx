...
{offerList && offerList.map(({currentClass, title, description}, index)  => (
  <li key={index} className={` ${currentClass} flex flex-col items-center gap-2 md:flex-row xl:gap-4`}>
    <OfferCart index={index} title={title} description={description}/>
</li>
))}
 </ul>
        </div>
