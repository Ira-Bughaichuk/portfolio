...
export const CartEducation = forwardRef<HTMLDivElement, ICartEducationProps>(( {custom, variants, university, profession, data, path }, ref) => {
  return (
    <motion.div ref={ref} variants={variants} custom={custom}
     className='font-condensed w-full h-full pb-[6px] mm:pb-3 md:pb-3  bg-color-btn-primary rounded-[10px]'>
    ...
    <a href={path} target="_blank" aria-label={profession} rel="noopener noreferrer nofollow" className="text-color-secondary transition duration-500 ease-in-out hover:text-color-accent-extra focus:text-color-accent-extra
